...
> **[해결 확인 — 2026-07-04, SFTF 쪽 검증]** 07-03 20:26 빌드
> (`SLOT_SUM_TYPE`=int32 누산기 + `SlotVo_32i`/`SlotVss_32i` 분리 저장)로 확인:
> E3_931902 3° 그리드 음수 7,348개 → **0개**(min 0.0, max 35,430 — CUDA 스케일과 일치),
> A1 cube 평면 자세 min 0.0 (해석적 기대치 일치). 단, C1 Bunny 등 **일반 메시도
> 전 셀 값이 ~15–20% 하향** (구 dll의 `sumType(typeTC)`이 높이 256 컬럼 z-합 32,640으로
> int16 한계 직하에서 상시 래핑) — 즉 구 dll 산출물 전체가 오염돼 있었음.
> SFTF 저장소는 전 그리드 재스윕 진행 (`scripts/resweep_int3_new_dll.py`).
> **CUDA 이슈 A(0-셀 레이스)·B(대형 메시 ~130× 슬로다운)는 이 수정에 포함되지 않아
> 여전히 미해결.**
>
> **[추가 해결 — 2026-07-04]** 부속 이슈 A·B 모두 07-04 빌드에서 수정·검증 완료.
> 근본 원인은 복셀화 슬롯 삽입의 스핀락 직렬화 + 용량 64 포화 시 스케줄링-의존
> 픽셀 드롭. 락-프리 CAS 삽입 + 결정적(삼각형 순서) 63픽셀 절단으로 교체.
> E3 10회 반복 완전 일치·스퓨리어스 0-셀 제거, E8 스윕 27,646s→~1,162s.
> 상세는 `TODO_CUDA_issues_2026-07-04.md` 상단 해결 노트 참고.
...
**07-04 재스윕 결과 (결정성 판정 완료)**: 같은 머신에서 1° 스윕을 다시 돌린 결과
**이번에도 정확히 7개의 0-셀**이 나왔고, 그중 3개는 동일 위치, 나머지는 **같은 두 방향
근방 안에서 셀만 이동** (run1: yaw 0–2/pitch 323, yaw 177–180/pitch 141–145;
run2: yaw 355–360/pitch 320–324, yaw 180–183/pitch 141–143).
또한 0이 아닌 셀들도 두 런이 완전 일치하는 것이 130,310개 중 20,773개뿐
(최대 차이 831) — **CUDA 백엔드가 런 간 비결정적**. float atomic 연산 순서
경합(예: cuda_src/atomicWrite.cu 경로)과 부합하며, 0-셀은 특정 기하 근방에서
확률적으로 터지는 커널 실패로 보임. 소요 시간은 20,037s → 19,943s로 재현
(슬로다운도 안정적).

증거 그리드: 이 폴더의 `Group_E3_931902_tomo_cuda_1deg_60deg.run1.npz`(첫 스윕),
run2는 `Tomo_SFTF_dev/Experimental/G5Test/tomo_cuda_cache/`의 동명 npz.
