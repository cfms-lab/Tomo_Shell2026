...
TPVector STPSlot::createVoPxls(const TPVector& alpha, const TPVector& beta)
{
  ...
  Vo.z = _max(Vo.z , INT16(0));//to reduce possible unexepcted noises.
  Vo.iTypeByte = typeVo;
  Vo_pxls.push_back(Vo);
  return Vo_pxls;
}

void _matchPairNumber_TMPxl(TPVector& al_slot, TPVector& be_slot)//주의:이 버전은 pxls가 z좌표 기준 내림차순이다.
{
  if (al_slot.size() > be_slot.size())
  {
    TPVector temp(al_slot.end() - be_slot.size(), al_slot.end()); al_slot = temp;//prefer Low Alpha. al_slot은 z값이 큰 쪽을 삭제.
  }
  else if (al_slot.size() < be_slot.size())
  {
    be_slot.resize(al_slot.size());//prefer High Beta. be_slot은 z값이 작은 쪽을 삭제. 
  }
}
  ...
  for(auto& nvb: nvB0)
  {
    ...
    if(_bUseExplicitSS)
    {
      ...
      for (int z_offset = nvb.z; z_offset >= new_nbA.z; z_offset--)
      {
        ...
        new_ss.z = z_offset;
        new_ss.iTypeByte = typeSS;//Caution: assign, not OR
...
void  STPSlot::Pairing(FLOAT32 theta_c, bool _bUseExplicitSS)
{
  ...
  if (_bUseExplicitSS)
  {
    ...
    Vss_pxls = createVss_Explicit( SSB_pxls, SSA_pxls);//#(for rendering)
  }
  else//implicit SS
  {
    createTCPxls();
    ...
    Vss_pxls = createVss_Implicit(al_pxls, be_pxls, TC_pxls, NVB_pxls, NVA_pxls, wFunc);//#(for rendering)
  }
#endif
  pxls.clear();
...
#ifdef _DEBUG
  //for (auto& pxl : SSB_pxls)   {     vm_info.SS_vol += pxl.z; vm_info.Ass_clad += 2; }
  //for (auto& pxl : SSA_pxls)   {     vm_info.SS_vol -= pxl.z; vm_info.Ass_clad += 2; }
#endif
        
  if (Vo_pxls.size() > 0)   {    vm_info.Vo = FLOAT32(Vo_pxls.begin()->z);   }
  if (Vss_pxls.size() > 0)  {    vm_info.Vss = FLOAT32(Vss_pxls.begin()->z); }
}
...
#if 1
TPVector  STPSlot::GetSSPxls(bool _bUseExplicitSS)
{
  if (_bUseExplicitSS)
  {
    return SS_pxls;
  ...
  for (TPIterator pIt = pxls.begin() ; pIt != pxls.end() ; pIt++)
  {
    ...
    if ((ss_start_z >= 0) && (pIt == pxls.end()- 1/*바닥면 직전일 때*/))
    {
      ss_end_z = 0;
    }
